...
import { Heart, Eye, Image as ImageIcon, Sparkles, Calendar } from "lucide-react";
...
async function getProfileData() {
  ...
  const posts = await db
    .collection("posts")
    .find({ userId: session.user.id, status: "published" })
    .sort({ createdAt: -1 })
    .toArray();
  ...
  // Check if we can get user creation date from DB for 'joined date'
  const userDoc = await db.collection("user").findOne({ id: session.user.id });
  
  return {
    user: {
      ...session.user,
      createdAt: userDoc?.createdAt || session.session.createdAt,
      bio: "AI Prompt Engineer & Creator", // Placeholder since we don't have bio editing yet
    },
...
export default async function ProfilePage() {
  const { user, posts, stats } = await getProfileData();
  ...
  return (
    <div className="min-h-screen bg-ugen-bg pt-28 pb-16 px-6 selection:bg-ugen-primary/30">
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        ...
        <div className="glass rounded-3xl p-8 border border-white/10 shadow-2xl mb-12 flex flex-col md:flex-row items-center md:items-start gap-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-1 md:gap-4">
            {posts.map((post: any, index: number) => (
              <Link 
                href={`/post/${post._id}`} 
                key={post._id}
                className="group relative aspect-square bg-black/40 overflow-hidden md:rounded-2xl border border-transparent md:border-white/5 hover:border-white/20 transition-all cursor-pointer"
              >
                <img
                  src={post.image}
                  alt={post.title}
